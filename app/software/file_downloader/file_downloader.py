...
def safe_extract_zip(zip_ref, target_dir):
    ...
    for member in zip_ref.infolist():
        ...
        if not member_path.startswith(target_dir + os.sep) and member_path != target_dir:
            raise ValueError(f"检测到非法路径，已阻止解压: {member.filename}")
...
class FileDownloader:
    def __init__(self, root):
        ...
        url_label = ttk.Label(url_frame, text="下载地址:")
    ...
    def download_file(self, url):
        try:
            ...
            # 如果是zip文件且选择了自动解压
            if file_name.lower().endswith('.zip') and self.auto_extract.get():
                self.status_label.config(text="正在解压文件...")
                with zipfile.ZipFile(download_path, 'r', encoding='gbk') as zip_ref:
                    safe_extract_zip(zip_ref, save_dir)
    ...
    def get_software_dir(self):
        ...
        try:
            ...
            # 如果是开发环境
            if os.path.basename(current_dir) == 'file_downloader':
                # 返回上级目录
